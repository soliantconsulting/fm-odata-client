...
describe("Database", () => {
    ...
    describe("schemaManager", () => {
        ...
        it("should throw error when called in a batched context", async () => {
            await expect(
                ...
                "Schema alterations are not allowed in a batch operation",
            );
        });
    });
    ...
    describe("listTables", () => {
        it("should return a list of tables", async () => {
            connectionStub.fetchJson.returns(
                Promise.resolve({
                    value: [
                        {
                            name: "baz",
                            kind: "EntitySet",
                            url: "https://localhost/fmi/odata/v4/foo/bar",
                        },
                    ],
                }),
            );
            const tables = await database.listTables();
            expect(tables).to.be.eql([
                { name: "baz", kind: "EntitySet", url: "https://localhost/fmi/odata/v4/foo/bar" },
            ]);
    ...
    describe("getMetadata", () => {
        it("should return response metadata", async () => {
            connectionStub.fetchJson.returns(
                Promise.resolve({
                    foo: {
                        baz: { $Kind: "EntityType", $Key: ["ID"], ID: { $Type: "Edm.Decimal" } },
                    },
                }),
            );
