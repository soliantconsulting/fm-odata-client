...
describe('SchemaManager', () => {
    ...
    describe('createTable', () => {
        it('should send create table request', async () => {
            ...
            const response = await schemaManager.createTable('foo', [{name: 'bar', type: 'string'}]);
            ...
            sinon.assert.calledWith(databaseStub.fetchJson, '/FileMaker_Tables', {
                method: 'POST',
                body: JSON.stringify({tableName: 'foo', fields: [{name: 'bar', type: 'varchar'}]}),
            });
        });

        it('should add repetitions to field type', async () => {
            await schemaManager.createTable('foo', [{name: 'bar', type: 'numeric', repetitions: 2}]);
            sinon.assert.calledWith(databaseStub.fetchJson, '/FileMaker_Tables', {
                method: 'POST',
                body: JSON.stringify({tableName: 'foo', fields: [{name: 'bar', type: 'numeric[2]'}]}),
            });
        });

        it('should add max length to string field', async () => {
            await schemaManager.createTable('foo', [{name: 'bar', type: 'string', maxLength: 10}]);
            sinon.assert.calledWith(databaseStub.fetchJson, '/FileMaker_Tables', {
                method: 'POST',
                body: JSON.stringify({tableName: 'foo', fields: [{name: 'bar', type: 'varchar(10)'}]}),
            });
        });

        it('should combine max length and repetitions', async () => {
            await schemaManager.createTable('foo', [{name: 'bar', type: 'string', maxLength: 10, repetitions: 2}]);
            sinon.assert.calledWith(databaseStub.fetchJson, '/FileMaker_Tables', {
                method: 'POST',
                body: JSON.stringify({tableName: 'foo', fields: [{name: 'bar', type: 'varchar(10)[2]'}]}),
            });
        });
    });
    ...
    describe('addFields', () => {
        it('should send add fields request', async () => {
            ...
            const response = await schemaManager.addFields('foo', [{name: 'bar', type: 'string'}]);
            expect(response).to.be.eql({
                tableName: 'foo',
                fields: [{name: 'bar', type: 'string'}],
            });
